...
Looking at Figure \ref{fig:kendalldistance} we can see the results of the SFD test. As mentioned in Section \ref{sec:distance}\note{mention this} the distance measures have a score between 0 and 1 where 0 correspond to equal lists and 1 that the lists are reverse of each other. There is a third case as the method we uses find an average approach meaning that if we have completely disjoint lists the distance score will be 0.78.
...
\begin{table}[H]
	...
	\end{tabular}
	\caption{Percentage increase between the groups for Kendall distance}
\end{table}