\subsubsection{Normalized Discounted Cumulative Gain}\label{sec:methodology_ndcg}
...
Conversely, it can be argued adjusted nDCG measure is inferior, as the aggregation methods, aside from Avg, only consider the ranked elements when making the aggregation, so the increased score is from the perspective of the aggregation methods entirely random.